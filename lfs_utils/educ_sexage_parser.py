"""
EDUC-SexAge Sheet Parser
Specialized parser for the EDUC-SexAge sheet with complex education level structure
"""
...
class EDUCSexAgeParser:
    """Specialized parser for EDUC-SexAge sheet with complex education level structure"""
    ...
    def parse_sheet(self, analysis: Dict[str, Any]) -> pd.DataFrame:
        """
        Parse EDUC-SexAge sheet into wide SDMX format
        ...
        """
        try:
            logger.info(f"Parsing EDUC-SexAge sheet '{analysis['sheet_name']}' into SDMX format")
            ...
            logger.error(f"Error parsing EDUC-SexAge sheet: {e}")
            raise
    ...
    def _create_column_mapping(self, analysis: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if len(df) > 2:
            ...
            # Map Education Level (columns 4-13)
            ...
            # Map Formal/Informal Education (columns 14-19)
            formal_informal_columns = [14, 15, 16, 17, 18, 19]
            ...
            # Map NEET (columns 20-24)
            neet_columns = [20, 21, 22, 23, 24]
            neet_names = [
            ...
            # Map Tertiary attainment 30-34 (column 25)
            ...
            # Map Lifelong learning 20-64 (column 26)
            if 26 < len(row2) and pd.notna(row2.iloc[26]):
    ...
    def create_sdmx_template(self, analysis: Dict[str, Any]) -> pd.DataFrame:
        ...
        logger.info("Creating SDMX template for EDUC-SexAge with all dimensions")
                ...
                for sex in sex_values:
                    for age in age_values:
                        ...
                        record = self._create_template_record(
                            year, sex, age, '_Z', '_Z', '_Z', '_Z', '_Z', '_Z'
                        )
                        ...
                        for main_level in education_main_levels:
                            for sub_level in education_sub_levels:
                                record = self._create_template_record(
                                    year, sex, age, main_level, sub_level, '_Z', '_Z', '_Z', '_Z'
                                )
                                template_records.append(record)
                        ...
                        for formal_cat in formal_informal_values:
                            record = self._create_template_record(
                                year, sex, age, '_Z', '_Z', formal_cat, '_Z', '_Z', '_Z'
                            )
                            template_records.append(record)
                        ...
                        for neet_cat in neet_values:
                            record = self._create_template_record(
                                year, sex, age, '_Z', '_Z', '_Z', neet_cat, '_Z', '_Z'
                            )
                            template_records.append(record)
                        ...
                        for tertiary_cat in tertiary_30_34_values:
                            record = self._create_template_record(
                                year, sex, age, '_Z', '_Z', '_Z', '_Z', tertiary_cat, '_Z'
                            )
                            template_records.append(record)
                        ...
                        for lifelong_cat in lifelong_20_64_values:
                            record = self._create_template_record(
                                year, sex, age, '_Z', '_Z', '_Z', '_Z', '_Z', lifelong_cat
                            )
                            template_records.append(record)
    ...
    def print_parsing_summary(self, df: pd.DataFrame, analysis: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"EDUC-SEXAGE DATA PARSING SUMMARY")
        print(f"{'='*80}")
