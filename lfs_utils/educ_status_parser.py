"""
EDUC-Status Sheet Parser
Specialized parser for the EDUC-Status sheet with employment status + education level structure
"""
...
class EDUCStatusParser:
    """Specialized parser for EDUC-Status sheet with employment status + education level structure"""
    ...
    def parse_sheet(self, analysis: Dict[str, Any]) -> pd.DataFrame:
        """
        Parse EDUC-Status sheet into wide SDMX format
        ...
        """
        try:
            logger.info(f"Parsing EDUC-Status sheet '{analysis['sheet_name']}' into SDMX format")
            ...
            logger.error(f"Error parsing EDUC-Status sheet: {e}")
            raise
    ...
    def _parse_data_structure(self, data_rows: pd.DataFrame, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for row_idx, row in data_rows.iterrows():
            # Each row represents a Year+Employment Status combination
    ...
    def _create_column_mapping(self, analysis: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if len(df) > 2:
            ...
            # Map Formal/Informal Education (columns 13-17)
            formal_informal_columns = [13, 14, 15, 16, 17]
            formal_informal_names = [
                'Total', 'in formal education', 'in informal education',
                'of them Employed', 'of them Unemployed'
            ]
            ...
            # Map NEET (columns 18-22)
            neet_columns = [18, 19, 20, 21, 22]
            neet_names = [
            ...
            # Map Tertiary attainment 30-34 (column 23)
            if 23 < len(row2) and pd.notna(row2.iloc[23]):
                mapping['tertiary_30_34'].append({
                    'column': 23,
                    'category': 'Tertiary educational attainment aged 30-34'
                })
            
            # Map Lifelong learning 20-64 (column 24)
    ...
    def _parse_employment_status(self, status_str: str) -> Tuple[str, str]:
        ...
        Handles main statuses and sub-categories with prefixes like '- ' and '-- '
        """
        status_str = status_str.strip()
        if status_str == 'Employed': return 'Employed', 'Total'
        elif status_str == 'Unemployed': return 'Unemployed', 'Total'
        elif status_str == 'Inactive': return 'Inactive', 'Total'
        elif status_str == 'TOTAL POPULATION AGED 15+': return 'TOTAL POPULATION AGED 15+', 'Total'
        if status_str.startswith('- '): return 'Employed', status_str[2:]
        elif status_str.startswith('-- '): return 'Employed', status_str[3:]
    ...
    def _parse_data_row_breakdowns(self, row: pd.Series, headers: Dict[str, Any], analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Parse employment status hierarchy
        main_employment_status, sub_employment_status = self._parse_employment_status(employment_status_str)
        ...
        if pd.notna(total_pop) and self._is_numeric(str(total_pop)):
            total_pop_record = self._create_base_record(year_str, main_employment_status, sub_employment_status)
        ...
        for edu_info in column_mapping['education_level']:
            ...
            if pd.notna(value) and self._is_numeric(str(value)):
                record = self._create_base_record(year_str, main_employment_status, sub_employment_status)
    ...
    def create_sdmx_template(self, analysis: Dict[str, Any]) -> pd.DataFrame:
        ...
        logger.info("Creating SDMX template for EDUC-Status with all dimensions")
        ...
        if data_struct['data_start_row'] is not None:
            ...
            employment_status_values = data_rows.iloc[:, 1].dropna().unique()
            ...
            for year in year_values:
                for employment_status in employment_status_values:
                    # Parse employment status for template
                    main_status, sub_status = self._parse_employment_status(str(employment_status))
                    
                    # Add total population record
                    record = self._create_template_record(
                        year, main_status, sub_status, '_Z', '_Z', '_Z', '_Z', '_Z', '_Z'
                    )
                    ...
                    for main_level in education_main_levels:
                        for sub_level in education_sub_levels:
                            record = self._create_template_record(
                                year, main_status, sub_status, main_level, sub_level, '_Z', '_Z', '_Z', '_Z'
                            )
                            template_records.append(record)
                    ...
                    for formal_cat in formal_informal_values:
                        record = self._create_template_record(
                            year, main_status, sub_status, '_Z', '_Z', formal_cat, '_Z', '_Z', '_Z'
                        )
                        template_records.append(record)
                    ...
                    for neet_cat in neet_values:
                        record = self._create_template_record(
                            year, main_status, sub_status, '_Z', '_Z', '_Z', neet_cat, '_Z', '_Z'
                        )
                        template_records.append(record)
                    ...
                    for tertiary_cat in tertiary_30_34_values:
                        record = self._create_template_record(
                            year, main_status, sub_status, '_Z', '_Z', '_Z', '_Z', tertiary_cat, '_Z'
                        )
                        template_records.append(record)
                    ...
                    for lifelong_cat in lifelong_20_64_values:
                        record = self._create_template_record(
                            year, main_status, sub_status, '_Z', '_Z', '_Z', '_Z', '_Z', lifelong_cat
                        )
                        template_records.append(record)
    ...
    def print_parsing_summary(self, df: pd.DataFrame, analysis: Dict[str, Any]):
        ...
        print(f"\n{'='*80}")
        print(f"EDUC-STATUS DATA PARSING SUMMARY")
        print(f"{'='*80}")
